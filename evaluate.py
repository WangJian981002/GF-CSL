...
parser.add_argument("--traindir", type=str, default='../DOTA/datasets/trainvalsplit-1024-256')
parser.add_argument("--test_image_dir", type=str, default='../datasets/testsplit/images')
...
def inference_testset(id_path_dict,out_dir,ori_size=1024,input_size=1120,root_dir=args.test_image_dir):
    ...
    model = ResNet(num_layers=agrs.model, heads=args.heads).cuda()
...
def eval_dota(out_id,input_size,is_flip=False,is_V=False):
    ...
    id_path_dict = genarate_img_subimage_dict(sub_img_dir='../datasets/test4000/images')
    if is_flip:
        if is_V:
