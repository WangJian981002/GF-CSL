...
parser.add_argument("--datadir", type=str, default='../DOTA/datasets/trainvalsplit-1024-256')
...
parser.add_argument("--lr",  default=1.25e-4)
...
parser.add_argument('--dist-url', default='tcp://127.0.0.1:2500', type=str,
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    train_loss = collections.deque(maxlen=1000)
    ...
    if args.resume:
        ...
        print("==>finished loading weight from: ",args.resume_weight_path)
    ...
    trainset = DotaSetv1(root_dir=args.datadir,img_size=args.input_size)
    ...
    for epoch in range(args.start_epoch,args.epochs) :
        ...
        if args.rank == 0 and (epoch + 1) % args.save_interval == 0:
            if not os.path.isdir('checkpoint'):
                os.mkdir('checkpoint')
            if not os.path.isdir('checkpoint/dota'):
                os.mkdir('checkpoint/dota')
            print("Saving...")
            torch.save(model.state_dict(), f"checkpoint/dota/ckpt_%d.pth" % (epoch+1))
