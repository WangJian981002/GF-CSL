...
class losses(nn.Module):
    ...
    def forward(self, heatmap,scale,offset,theta,heatmap_t,label,smoothlabel):
        ...
        transformed_smoothlabel = smoothlabel.clone()
        transformed_smoothlabel = torch.cat([transformed_smoothlabel[:, 90:], transformed_smoothlabel[:, 0:90]], dim=1)
        aware_transformed_smoothlabel = torch.clamp((-2.5*(label_new[:, 3] / label_new[:, 4]) + 3.5).view(N,1).cuda(), min=0.0, max=1.0) * transformed_smoothlabel  #linear
        #aware_transformed_smoothlabel = (0.5+0.5*torch.cos(2.5*math.pi*(torch.clamp(label_new[:, 3] / label_new[:, 4], min=1.0, max=1.4)-1))).view(N,1).cuda() * transformed_smoothlabel  #cosine
        smoothlabel = torch.max(smoothlabel, aware_transformed_smoothlabel)
